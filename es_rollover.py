...
def main():
    if len(sys.argv) < 3:
        print(
            'USAGE: [INDEX_PREFIX=(default "")] [ARCHIVE=(default false)] ... {} ACTION http://HOSTNAME[:PORT] yes/no(rollover_service_indices)'.format(
                sys.argv[0]))
        print('ACTION ... one of:')
        ...
        print(
            '\tSHARDS ...  the number of shards per index in Elasticsearch (default {}).'.format(
                SHARDS))
        print(
            '\tREPLICAS ... the number of replicas per index in Elasticsearch (default {}).'.format(
                REPLICAS))
        print('rollover configuration:')
        print(
            '\tCONDITIONS ... conditions used to rollover to a new write index (default \'{}\'.'.format(
                ROLLBACK_CONDITIONS))
    ...
    client = create_client(os.getenv("ES_USERNAME"), os.getenv("ES_PASSWORD"),
                           str2bool(os.getenv("ES_TLS", 'false')),
                           os.getenv("ES_TLS_CA"), os.getenv("ES_TLS_CERT"),
    ...
    action = sys.argv[1]
    rollover_services = True if action == "init" or (
            len(sys.argv) > 3 and sys.argv[3] == "yes") else False

    if str2bool(os.getenv('ARCHIVE', 'false')):
        write_alias = prefix + ARCHIVE_INDEX + '-write'
        read_alias = prefix + ARCHIVE_INDEX + '-read'
        perform_action(action, client, write_alias, read_alias,
                       prefix + 'jaeger-span-archive', 'jaeger-span', prefix)
    else:
        write_alias = prefix + 'jaeger-span-write'
        read_alias = prefix + 'jaeger-span-read'
        perform_action(action, client, write_alias, read_alias, prefix + 'jaeger-span',
                       'jaeger-span', prefix)

        if rollover_services:
            write_alias = prefix + 'jaeger-service-write'
            read_alias = prefix + 'jaeger-service-read'
            perform_action(action, client, write_alias, read_alias,
                           prefix + 'jaeger-service', 'jaeger-service', prefix)


def perform_action(action, client, write_alias, read_alias, index_to_rollover,
...
def create_index_template(template, template_name):
    ...
    s = get_request_session(os.getenv("ES_USERNAME"), os.getenv("ES_PASSWORD"),
                            str2bool(os.getenv("ES_TLS", 'false')),
                            os.getenv("ES_TLS_CA"), os.getenv("ES_TLS_CERT"),
                            os.getenv("ES_TLS_KEY"),
...
def create_aliases(client, alias_name, archive_index_name, use_ilm):
    ...
    if re.search(r'write', alias_name) and use_ilm:
        alias = curator.Alias(client=client, name=alias_name,
                              extra_settings={'is_write_index': True})
    else:
        alias = curator.Alias(client=client, name=alias_name)
    alias.add(ilo)
    alias.do_action()
...
def read_alias_lookback(client, write_alias, read_alias, unit, unit_count):
    ...
    ilo.filter_by_age(source='creation_date', direction='older', unit=unit,
...
def fix_mapping(template_name, esVersion, shards, replicas, indexPrefix, use_ilm,
    ...
    output = subprocess.Popen(
        ['esmapping-generator', '--mapping', template_name, '--es-version',
         str(esVersion),
...
def create_client(username, password, tls, ca, cert, key, skipHostVerify, timeout):
    ...
    if username is not None and password is not None:
        return elasticsearch.Elasticsearch(sys.argv[2:], http_auth=(username, password),
...
def check_if_ilm_policy_exists(ilm_policy):
    ...
    if r.status_code != 200:
        sys.exit(
            "ILM policy '{}' doesn't exist in Elasticsearch. Please create it and rerun init".format(
                ilm_policy))
